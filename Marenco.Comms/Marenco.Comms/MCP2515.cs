 ...
 * 
 * Modified by Becker for use in Lockwing.
 * Added fliters and masks for RB0 only.
 * Added interrupt as standard. Need to clear the interrupt flag
...
namespace Marenco.Comms
{
    ...
    public class MCP2515
    {
        ...
        //        public const Cpu.Pin SLAVESELECT = (Cpu.Pin) Pins.GPIO_PIN_A0; // was 26, now 54
        ...
        //
        //  A debug function by Becker
        //

        public void CheckRegisters()
        {
            byte b = ReadRegister(RXB0CTRL);

        }

        //  Needed because we are enabling the Can interrupt.

        public void ResetCanInterrupt()
        {
        ...
        public void SetMask(UInt16 filter, UInt16 mask)
        {
            ...
            CheckRegisters();
            WriteRegister(RXB0CTRL, (byte)0x20);
            byte[] thing = new byte[] { 0x00 };
            spi.WriteRead(new byte[] { 0x03, RXB0CTRL }, thing, 2);
            byte b = ReadRegister(RXB0CTRL);

            //  Set the mask. A 1 in the mask bit means that the filter will
            //  be applied to this bit.
            //  SIDH contains the high mask bits. SIDL contains the
            //  low mask bits in slots 5, 6 and 7.

            byte high = (byte)(mask >> 3);
            byte low = (byte)(mask & 0x007);

            WriteRegister(RXM0SIDH, high);
            b = ReadRegister(RXM0SIDH);
            WriteRegister(RXM0SIDL, (byte)(low << 5));
            b = ReadRegister(RXM0SIDH);
